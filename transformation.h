...
Vec2 World_To_Pixel(const Vec3& source ,const Vec3& camera, const Vec3& LookTo,float planeWidth, float planeHeight, float winWidth, float winHeight,int flag){
...
//    //calculate the screen pixels
    ...
    retVal.y = (int)(retVal.y * winHeight);
...
void RotateZ(Vec3& Point,float theta){
    ...
    float angle = theta/180*pi;
    //Transformation matrix
    T(0,0) = cos(angle);    T(0,1) = -sin(angle);       T(0,2) = 0;    T(0,3) = 0;
    T(1,0) = sin(angle);    T(1,1) = cos(angle);        T(1,2) = 0;    T(1,3) = 0;
